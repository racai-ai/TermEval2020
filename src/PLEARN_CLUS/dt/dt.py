...
for corpus in ["wind","corp"]:
    
    loadCorpusData(corpus)
    
    c=MyCorpus(dir="../my/en/corenlp/%s/"%(corpus),sampleSize=3,startToken=1,embeddings=[],extension=".conll",forLSTM=False)
...
for corpus in ["corp","wind","equi"]:
    ...
    loadCorpusData(corpus)
    foundTerms={}
    c=MyCorpusTest(dir="../my/en/corenlp/%s/"%(corpus),sampleSize=3,startToken=1,embeddings=[],extension=".conll",forLSTM=False)
    ...
    fout=Path("../my/en/dt1/%s/annotations_terms.ann"%(corpus)).open(mode="w")
