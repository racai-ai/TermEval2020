...
#        else: ret.append(0)
    ...
    def getMapY(self,line,index):
        if(index==self.startToken+1):
            if(self.currentLines[index-1][1].lower()+" "+line[1].lower() in terms.keys()):
                return [1]
            else:
                return [0]
        return []
...
class MyCorpusTest(MyCorpus):
    def getMapX(self,line,index):
        if(index==self.startToken+1):
            testWords.append(self.currentLines[index-1][1].lower()+" "+line[1].lower())
...
for corpus in ["corp","wind"]:
    ...
    c=MyCorpus(dir="../my/en/corenlp/%s/"%(corpus),sampleSize=6,startToken=2,embeddings=[],extension=".conll",forLSTM=False)
...
for corpus in ["corp","wind","equi"]:
    ...
    foundTerms={}
    c=MyCorpusTest(dir="../my/en/corenlp/%s/"%(corpus),sampleSize=6,startToken=2,embeddings=[],extension=".conll",forLSTM=False)
    ...
    fout=Path("../my/en/dt2/%s/annotations_terms.ann"%(corpus)).open(mode="w")
