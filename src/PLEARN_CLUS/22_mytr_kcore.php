...
function processFileAnnotate($fname){
	...
	for($wordIndex=0;$wordIndex<count($words);$wordIndex++){
		...
		$score=0;$snum=0;
		foreach(array_slice($words,$wordIndex,$last-$wordIndex+1) as $word)
		    if(isset($terms[$word])){$score+=$terms[$word];$snum++;}
		if($snum>0)$score=floatval($score)/floatval($snum);
		$ann[$t]=$score;
	    }
...
foreach(["equi","corp","wind"] as $corpus){
    ...
    saveStat($ann,"my/$CORPUSLANG/mytr/$corpus/annotations_terms.stat");
}
